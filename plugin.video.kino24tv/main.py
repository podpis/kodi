...
cache = StorageServer.StorageServer("kino24tv")


import resources.lib.kino24tv as kino24tv
...
kino24tv.COOKIEFILE=os.path.join(DATAPATH,'kino24tv.cookie')
kino24tv.BRAMKA = ''
if my_addon.getSetting('bramka')=='true':
    kino24tv.BRAMKA = 'http://www.bramka.proxy.net.pl/index.php?q='
...
def ListMovies(ex_link,Type=''):
    filmy,pagination = kino24tv.scanMainpage(ex_link,Type)
    print '$$$$$$$$$$$$$$$$$$$$$$',filmy
    if pagination[0]:
        addLinkItem(name='[COLOR blue]<< poprzednia strona <<[/COLOR]', url=pagination[1], mode='__page__M',  IsPlayable=False)
    items=len(filmy)
    for f in filmy:
...
def ListSeriale(ex_link,page):
    filmy,pagination = kino24tv.scanMainpage(ex_link)
    if pagination[0]:
...
def ListEpisodes(ex_link):
    episodes = kino24tv.scanTVshow(ex_link)
    for f in episodes:
...
def getLinks(ex_link):
    links = kino24tv.getVideoLinks(ex_link)
    stream_url=''
...
if mode is None:
    addDir(name="[COLOR blue]Filmy[/COLOR]",ex_link='http://kino24-tv.pl/kategoria/filmy-online/',page=1, mode='ListMovies',iconImage='DefaultFolder.png',fanart=FANART)
    addDir(name="  [COLOR lightblue]Polecane[/COLOR]",ex_link='http://kino24-tv.pl', mode='ListMoviesPolecane',iconImage='DefaultFolder.png',fanart=FANART)
    addDir(name="  [COLOR lightblue]Ostatnio dodane[/COLOR]",ex_link='http://kino24-tv.pl',page=1, mode='ListMovies',iconImage='DefaultFolder.png',fanart=FANART)
    #addDir(name="Gatunki",ex_link='http://kino24-tv.pl',page=1, mode='Gatunek',iconImage='DefaultFolder.png',fanart=FANART)
    ...
    #addDir(name="[COLOR green]Seriale[/COLOR]",ex_link='http://cda-online.pl/seriale/',page=1, mode='ListSeriale',iconImage='DefaultFolder.png',fanart=FANART)
...
elif mode[0] == 'ListMoviesPolecane':
    ListMovies(ex_link,'polecane')

elif mode[0] =='Gatunek':
    data = kino24tv.Gatunki()
    for f in data:
        addDir(name=f.get('title'), ex_link=f.get('href'), mode='ListMovies', iconImage=f.get('img'), infoLabels=f)  
...
#     ListSeriale(ex_link,page)
    ...
    param = ex_link.split('|')
    data = kino24tv.getGatunekRok(rodzaj=param[0],typ=param[1])
    if data:
    ...
    if not historia == ['']:
        for entry in historia:
            ...
            addDir(name=entry, ex_link='http://kino24-tv.pl/?s='+entry.replace(' ','+'), mode='ListMovies', fanart=None, contextmenu=contextmenu) 
    ...
    if d:
        HistoryAdd(d)
        ex_link='http://kino24-tv.pl/?s='+d.replace(' ','+')
        ListMovies(ex_link,'')
